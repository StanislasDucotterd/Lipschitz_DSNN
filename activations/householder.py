...
class HouseHolder_Order_2(nn.Module):
    def __init__(self, channels):
        ...
        self.theta0 = nn.Parameter((np.pi * torch.rand(self.num_groups)), requires_grad=True)
        self.theta1 = nn.Parameter((np.pi * torch.rand(self.num_groups)), requires_grad=True)
        self.theta2 = nn.Parameter((np.pi * torch.rand(self.num_groups)), requires_grad=True)
